...
def _from_lr_function(lr_fn, *args):
  """Compatibility layer: create a learning rate from lr_functions function."""
  def learning_rate(step):
...
@gin.configurable(blacklist=['history'])
def constant(history, constant_value):
  del history
  return _from_lr_function(lr_functions.constant, constant_value)


@gin.configurable(blacklist=['history'])
def warmup(history, n_warmup_steps, constant_value):
  del history
  return _from_lr_function(lr_functions.warmup, n_warmup_steps, constant_value)
